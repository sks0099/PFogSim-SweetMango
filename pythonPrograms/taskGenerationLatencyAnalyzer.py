...
def getDistanceKmBetweenTwoPointsInLongLat(sourceLong, sourceLat, destLong, destLat):
    sourceX, sourceY = convertLongLatToXY(sourceLong, sourceLat)
    destX, destY = convertLongLatToXY(destLong, destLat)
    return math.sqrt(math.pow((sourceX-destX),2)+math.pow((sourceY-destY),2))/1000.0

def main():
    # file_path = "../sim_results_100_600/csvs/"
    # file_name = "1761835027688_KASUS.csv"
    file_path = "../sim_results_100_600/csvs/ite9/"
    file_name = "1765220552202_600_100_600_KASUS.csv"

    # file_path = "../sim_results/csvs/ite0/"
    # file_name = "1761912600405_5500_500_5500_node038.csv"
    output_file_path = 'taskAnalysisPlots/ite9/'
    ...
    df['maxDelay'] = df['maxDelay'].astype(float)
    df['taskSourceDestDistKm'] = df['taskSourceDestDistKm'].astype(float)
    # Set the option to display all columns
    pd.set_option('display.max_columns', None)
    # print(df[['taskLocLong','taskLocLat','taskDestLocLong','taskDestLocLat','taskSourceDestDistKm']])
    df['taskSourceDestDistKm'] = df.apply(lambda row: getDistanceKmBetweenTwoPointsInLongLat(row['taskLocLong'], row['taskLocLat'],
                                row['taskDestLocLong'], row['taskDestLocLat']),axis=1)
    df_filtered = df[df['taskSourceDestDistKm']>0.0001]
    print(df_filtered[['taskType', 'maxDelay','taskSourceDestDistKm']])
