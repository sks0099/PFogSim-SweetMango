...
public class VoronoiSingleLayerOrchestrator extends EdgeOrchestrator {
    ...
    boolean preferredHostIndexAscending = false;
    ...
    // unassignedMobileDeviceCount keeps track of the number of mobile devices that could be assigned a host.
    ...
    public void initialize() {
        ...
        for (Datacenter node : SimManager.getInstance().getLocalServerManager().getDatacenterList()) {
            ...
            if(((EdgeHost) node.getHostList().get(0)).getId() == 2) currentMIPS = ((EdgeHost) node.getHostList().get(0)).getTotalMips();
    ...
    /*private ArrayList<EdgeHost> getProspectiveNodes(MobileDevice mobile, Integer maxProspectiveNodeCount){
        ...
        Integer MaxHops = 5;
        Boolean prospectiveNodesFound = false;
        Integer prospectiveNodeCount = 0;
        if(nearest.isMIPSCapacitySufficient(mobile) && goodHost(nearest, mobile)) {
            prospectiveNodes.add(nearest);
            prospectiveNodeCount++;
            if(prospectiveNodeCount == maxProspectiveNodeCount) return prospectiveNodes;
        }
        while(!prospectiveNodesFound){
            if(prospectiveNodeCount == maxProspectiveNodeCount) prospectiveNodesFound = true;
        }
    ...
    private Map<Integer, Integer> getRank(Map<Integer, Double> inputMap){
        ...
        /*for (int i = 0; i < inputMap.size(); i++) {
            RankedNumber current = rankedNumbers.get(i);
            if (current.getValue() > previousValue) {
                currentRank.getAndAdd(tieCount); // Increment rank by tieCount if no tie
                current.setRank(currentRank.getAndIncrement());
                tieCount = 0; // Reset tie count
            } else {
                current.setRank(currentRank.get()); // Assign same rank for ties
                tieCount++;
            }
    ...
    public void assignHost(MobileDevice mobile) {
        ...
        if(preferredHostIndexAscending) {
            preferredHostListsIndex = preferredHostMinIndex;
            while (preferredHostListsIndex < (preferredHostMaxIndex+1)
                    && prospectiveNodes.size() == 0) {
        ...
        if(prospectiveNodes.size()==0){
            System.out.println("No host could be assigned.");
            //System.exit(880808);
        }
